(function() {
              ...
              $(function () {
                ...
                if (session.RTCSupported) {
                  require(["webrtc"], function (webrtc) {
                    ...
                    const isConnected = sessionStorage.getItem("connected") || "false"
  ...
  define('ui',["require", "jquery", "util", "session", "templates", "templating", "linkify", "peers", "windowing", "tinycolor", "elementFinder", "visibilityApi"], function (require, $, util, session, templates, templating, linkify, peers, windowing, tinycolor, elementFinder, visibilityApi) {
    ...
    function bindMenu() {
      ...
      if (el.length) {
        ...
        el.css({
          bottom: bound.height() - $window.scrollTop() + "px",
          left: (boundOffset.left + bound.width() - 20 - el.width() - $window.scrollLeft()) + "px"
        });
      }
    }
    ...
    function adjustDockSize(buttons) {
      ...
      assert(newHeight >= BUTTON_HEIGHT * 4, "Height went too low (", newHeight,
  ...
  */
  ...
  define('webrtc',["require", "jquery", "util", "session", "ui", "peers", "storage", "windowing"], function (require, $, util, session, ui, peers, storage, windowing) {
    ...
    session.on("ui-ready", function () {
      $("#togetherjs-videochat-button").click(function () {
        if (session.RTCSupported) {
          enableVideo();
        } else {
      ...
      $("#togetherjs-audio-button").click(function () {
        ...
        const isConnected = obj[sessionStorage.getItem("connected")]
      ...
      function disableAudio() {
        ...
        if (_connection) {
          _connection.getRemoteStreams().forEach(remoteStream => {
            remoteStream.getTracks().forEach(track => {
              track.stop();
            })
          })

          _connection.getLocalStreams().forEach(localStream => {
      ...
      webrtc.enableAudio = enableAudio
      ...
      function createVideoStreamElement(videoStream) {
      ...
      function appendVideoStreamElement(parentElement, video) {
        parentElement.appendChild(video);
      }

      function setAttributes(nodeObject, attributes) {
        for (const attribute in attributes) {
          nodeObject.setAttribute(attribute, attributes[attribute])
        }
      }

      function setStyles(nodeObject, styles) {
        for (const style in styles) {
          nodeObject.style[style] = styles[style]
        }
      }
      ...
      function startVideoStreaming() {
        getUserMedia({
          ...
          const video = createVideoStreamElement(stream)
          videoStream = stream
          // const personId = session.clientId.replace(".", "_")
          const container = document.querySelector('.togetherjs-person-self')
          setStyles(container, {
            position: "relative"
          })
          // const container = document.querySelector(`#togetherjs-person-${personId}`)
          appendVideoStreamElement(container, video)
          videoConnection()
        }, function (err) {
      ...
      webrtc.getConnection = getConnection
      ...
      function getVideoConnection() {
        ...
        _connection.onaddstream = function (event) {
          console.log("GOT EVENT", event, event.type);
          const video = createVideoStreamElement(event.stream)
          appendVideoStreamElement(document.querySelector('#togetherjs-container'), video)
          // attachMedia($audio, event.stream);
          // audioButton("#togetherjs-audio-active");
        };
        _connection.onstatechange = function () {
        ...
        _connection.addStream(videoStream);
        return _connection;
      }

      function videoConnection () {
        const connection = getVideoConnection()
